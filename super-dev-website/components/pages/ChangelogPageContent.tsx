...
const CHANGELOG = {
  zh: [
    { version: '2.1.2', date: '2026-03-24', type: 'major' as const, changes: ['新增 OpenClaw 原生插件（@super-dev/openclaw-plugin），通过 Plugin SDK 注册 10 个专用 Tool', '宿主矩阵扩展至 21 个（新增 OpenClaw 为第 29 个宿主）', '修复 50+ 个 bug：delete_spec 删错目录、Prisma nullable 反转、TypeORM 语法错误、CLI console 崩溃等', '新增 backend/src/app.js 入口文件和 frontend/src/App.tsx 根组件', '补全 3 个缺失的后端路由（core/experience/operation）', '修复 7 个后端 repository 的 ID 覆盖漏洞和生成策略', '修复 HSL 色彩算法、asyncio 弃用 API、SQLAlchemy 迁移语法等'] },
