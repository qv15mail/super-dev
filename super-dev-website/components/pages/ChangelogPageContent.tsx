...
const CHANGELOG = {
  zh: [
    { version: '2.0.11', date: '2026-03-20', type: 'patch' as const, changes: ['对外主推宿主矩阵收敛到 16 个热门宿主，减少实验宿主带来的不稳定预期', '新增显式 Bugfix Mode（super-dev fix），修复缺陷时走轻量补丁路径', '新增 Repo Map、Impact、Regression Guard、Dependency Graph，形成完整代码库理解链路', '新增 Host Validation Center 能力：宿主前置条件诊断、运行时验收状态和交付就绪摘要', '新增 integrate harden、spec scaffold / quality、run --status / --phase / --jump / --confirm 等流程控制指令', '升级 Proof Pack，增加 executive summary、blockers、next actions、summary 输出，并将 Spec Quality 接入统一发布评分面板'] },
  ...
  en: [
    { version: '2.0.11', date: '2026-03-20', type: 'patch' as const, changes: ['Focused the public host matrix on 16 primary hosts instead of advertising unstable lab adapters', 'Added explicit Bugfix Mode through super-dev fix so defect work follows a lightweight patch path', 'Added Repo Map, Impact, Regression Guard, and Dependency Graph as a complete codebase-intelligence chain', 'Added Host Validation Center capabilities for host prerequisites, runtime acceptance state, and delivery readiness summaries', 'Added workflow-control commands including integrate harden, spec scaffold / quality, and run --status / --phase / --jump / --confirm', 'Upgraded Proof Pack with executive summary, blockers, next actions, and summary output, and added Spec Quality into the unified release panel'] },
