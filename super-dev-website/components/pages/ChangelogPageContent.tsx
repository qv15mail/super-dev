...
const CHANGELOG = {
  zh: [
    { version: '2.1.4', date: '2026-03-26', type: 'patch' as const, changes: ['修复 Windows 下 uv tool upgrade 文件锁定问题（自动重试 + 清晰提示）', '新增 super-dev clean 命令（清理历史产物，支持 --keep/--all/--dry-run）', '6 人 Agent 团队全项目深度审查，修复 37 个 bug（含 3 个 P0 崩溃、12 个 P1 逻辑错误）', '修复 proof-pack rehearsal 状态始终为 ready 的问题', '修复 Sequelize/Jenkins/TypeORM 生成代码的语法错误', '新增路径遍历安全防护（specs delete/change 操作）', '质量评分计算从整数截断改为四舍五入', '官网新增案例展示页（20+ 个真实项目）', '官网更新日志补全全部 18 个历史版本（1.0.0 ~ 2.1.4）'] },
  ...
  en: [
    { version: '2.1.4', date: '2026-03-26', type: 'patch' as const, changes: ['Fixed Windows uv tool upgrade file lock issue (auto-retry + clear guidance)', 'Added super-dev clean command (clean historical artifacts with --keep/--all/--dry-run)', '6-agent team deep audit: fixed 37 bugs (3 P0 crashes, 12 P1 logic errors)', 'Fixed proof-pack rehearsal status always showing ready', 'Fixed Sequelize/Jenkins/TypeORM generated code syntax errors', 'Added path traversal security protection for specs delete/change operations', 'Quality scoring changed from integer truncation to rounding', 'Added showcase page with 20+ real project cases', 'Changelog now includes all 18 historical versions (1.0.0 ~ 2.1.4)'] },
