...
const CHANGELOG = {
  zh: [
    { version: '2.2.0', date: '2026-03-29', type: 'major' as const, changes: [
      '知识推送引擎：每阶段自动推送相关知识约束（306文件索引，7阶段映射，L1/L2/L3渐进式加载）',
      '知识自演化：SQLite追踪使用效果，数据驱动权重优化',
      '可编程验证规则引擎：25条YAML声明式规则（14默认+11红队），支持自定义',
      '质量顾问：主动质量建议，Quick Wins优先排序',
      '专家系统四层武装：Profile+Knowledge+Rules+Protocol，11个深度Playbook',
      '交叉审查引擎：多专家视角自动审查交付物',
      'Spec-Code一致性检测：防止代码偏离Spec描述',
      'Pipeline效能度量：DORA五指标+Rework Rate',
      'ADR自动生成：架构决策自动记录',
      'Prompt模板版本化：文档生成Prompt可迭代优化',
      '外部审查集成：支持CodeRabbit/Qodo审查结果导入',
      'A2A协议支持：5个Agent Cards，支持外部Agent调用',
      '结构化输出Schema：5个JSON Schema定义，确保治理输出可验证',
      'Web API认证：X-Super-Dev-Key环境变量控制',
      'OpenClaw插件升级至20个Tool，覆盖全部核心命令',
      '知识库扩展至270+文件/15万行，覆盖23个技术领域',
      'P0修复：resume恢复上下文/intelligence真搜索/质量门禁深度检查/API认证',
      '终端UI自适应：Table和Panel随窗口大小自动调整',
      '测试覆盖：1600+测试，Python代码76K+行',
    ] },
  ...
  en: [
    { version: '2.2.0', date: '2026-03-29', type: 'major' as const, changes: [
      'Knowledge Push Engine: auto-push relevant knowledge per pipeline phase (306 files indexed, 7-phase mapping, L1/L2/L3 progressive loading)',
      'Knowledge Evolution: SQLite-based usage tracking with data-driven weight optimization',
      'Programmable Validation Rules: 25 YAML declarative rules (14 default + 11 redteam), customizable',
      'Quality Advisor: proactive quality suggestions with Quick Wins prioritization',
      'Expert System 4-layer upgrade: Profile+Knowledge+Rules+Protocol, 11 deep Playbooks',
      'Cross-Review Engine: multi-expert perspective artifact validation',
      'Spec-Code Consistency Checker: detect code drift from spec descriptions',
      'Pipeline Metrics: DORA 5 indicators + Rework Rate',
      'ADR Auto-Generation: automatic architecture decision records',
      'Prompt Template Versioning: iterative prompt optimization for doc generation',
      'External Review Integration: CodeRabbit/Qodo review result import',
      ...
      'OpenClaw plugin upgraded to 20 Tools covering all core commands',
