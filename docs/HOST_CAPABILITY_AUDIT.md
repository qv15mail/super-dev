...
| CodeBuddy IDE | native slash + official skills | `/super-dev 你的需求` | [CodeBuddy IDE integrations](https://www.codebuddy.ai/docs/cli/ide-integrations) / [CodeBuddy skills](https://www.codebuddy.ai/docs/cli/skills) | IDE 命令映射可用，skills 面已公开 |
...
| Trae | official project rules + compatibility skill | `super-dev: 你的需求` | [Trae rules](https://www.traeide.com/docs/what-is-trae-rules) | 公开文档确认 rules；`~/.trae/skills` 仅作为兼容增强路径协同生效 |
...
2. 不支持 slash 的宿主：
   - 直接输入 `super-dev: 你的需求`
3. CodeBuddy、Qoder、Windsurf、OpenCode、iFlow 这些宿主，官方文档已经公开 skills 面；Super Dev 现在按“命令触发 + Skills 协议”建模。
4. Codex CLI 当前按 official AGENTS.md + official skills 协同，用户不需要手动激活 Skill。
5. Kimi CLI 当前按 official AGENTS.md + text trigger / 文本触发建模，不再继续假设项目级自定义 slash。
6. Trae 当前收敛到 official project rules + compatibility skill；Kiro IDE 仍走 steering/rules；Qoder IDE 已收正为 commands + rules + official skills。
