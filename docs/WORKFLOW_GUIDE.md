...
1. 先修复质量 / 安全问题
2. 重新执行 quality gate 与 `super-dev release proof-pack`
3. 通过后再继续交付或恢复执行
...
super-dev bootstrap --name my-project --platform web --frontend react --backend node --domain saas
```
...
super-dev feature-checklist            # 审计 PRD 全量功能覆盖率，确认流程完成不等于范围完成
super-dev dependency-graph             # 生成依赖图与关键路径，适合大仓库或重构前分析
super-dev impact "修改登录流程" --files services/auth.py  # 先评估改动影响范围
super-dev regression-guard "修改登录流程" --files services/auth.py  # 把影响分析转换成回归检查清单
...
它们现在还会纳入 `feature-checklist` 的范围覆盖率结果，用来区分“流程完成”和“PRD 全量范围完成”。

如果你需要确认宿主是否真的完成了首轮 research、三文档落盘与确认门停顿，可继续执行：

```bash
super-dev integrate validate --auto
```

如果你发现系统提示“流程完成”，但文档或差距审计里仍列出很多未实现项，请执行：
