...
def migrate_project(project_dir: Path) -> list[str]:
    ...
    if config_path.exists():
        try:
            ...
            if old_version != "2.3.2":
                data["version"] = "2.3.2"
                config_path.write_text(
                    yaml.dump(data, allow_unicode=True, default_flow_style=False),
                    encoding="utf-8",
                )
                changes.append(f"super-dev.yaml 版本号 {old_version} -> 2.3.2")
    ...
    if claude_md.exists():
        try:
            ...
            if written:
                changes.append("已重新生成 .claude/CLAUDE.md（含 2.3.2 编码约束）")
        except Exception as exc:
    ...
    if skill_candidates:
        try:
            ...
            changes.append("已重新生成 SKILL.md（含 2.3.2 内容）")
        except Exception as exc:
