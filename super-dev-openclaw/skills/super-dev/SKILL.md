...
- **插件（Plugin）**：桥梁，将 20 个 Tool 注册到 OpenClaw，每个 Tool 调用本地 `super-dev` CLI
...
| `super_dev_governance` | 治理管理 | action: status/rules-list/rules-validate/knowledge-report |
| `super_dev_task` | 任务管理 | action: list/run/status, changeId |
| `super_dev_fix` | 缺陷修复 | description: bug 描述 |
| `super_dev_confirm` | 快捷门禁确认 | gate: docs/ui/architecture/quality/preview |
| `super_dev_jump` | 阶段跳转 | stage: 阶段名或编号 |
| `super_dev_product_audit` | 产品审查 | 无 |
| `super_dev_metrics` | 度量查看 | format: text/json |
